    ...
    <div style="margin-top: -4rem;" class="q-pa-lg">
      <div class="row">
        <div class="col-12 ">
          ...
          <q-table style="margin-top: 2rem;" title="Apps" flat :filter="searchKey" :loading="paginationUtil.loading"
            v-model:pagination="paginationUtil.pagination" :rows="appList" :columns="columns" row-key="memberId"
            padding>
                ...
                <q-btn-dropdown color="grey-8" dense>
                  <q-list>
                    ...
                    <q-item clickable @click="goToAssignedApps(props.row as UAMUserData)">
                      ...
                      <q-item-section>
                        <q-item-label>View Roles</q-item-label>
                      </q-item-section>
                    </q-item>
                  </q-list>
                </q-btn-dropdown>
            ...
            <q-step :name="1" title="User Information" icon="settings" :done="step > 1">
              <q-form @submit="onProcessApp" @reset="onReset">
                <div class="row q-col-gutter-x-lg  ">
                  <div class="col-md-6">
                    <q-input v-model="appPayload.app_name" label="App Name *" lazy-rules
                      :rules="[val => val && val.length > 0 || 'Enter the app\'s name']" />
                  </div>
                  <div class="col-md-6">
                    <q-input v-model="appPayload.caption" label="Caption  *" lazy-rules
                      :rules="[val => val && val.length > 0 || 'Enter the app\'s caption']" />
                  </div>
                  <div class="col-md-6">
                    <q-input v-model="appPayload.endpoint" label="Home URL  *" lazy-rules
                      :rules="[val => val && val.length > 0 || 'Enter the app\'s url']" />
                  </div>
                  <div class="col-md-6">
                    <q-input v-model="appPayload.app_key" label="App Key  *" lazy-rules
                      :rules="[val => val && val.length > 0 || 'Enter the app\'s key']" />
                  </div>
                  <div class="col-md-6">
                    <q-input v-model="appPayload.rank" label="Rank  *" type="number" lazy-rules :rules="[
            (val) => (val && Number(val) && val > 0) || 'Enter the app\'s rank'
          ]" />
                  </div>
                  <div class="col-md-6">
                    <q-input v-model="appPayload.description" label="Description  *" lazy-rules
                      :rules="[val => val && val.length > 0 || 'Enter the app\'s description']" />
                  </div>
                </div>
                <q-stepper-navigation>
                  <q-btn color="primary" label="Create App" type="submit" :loading="loading" v-if="isUpdate == false" />
                  <q-btn color="primary" label="Update App" type="submit" v-if="isUpdate == true" :loading="loading" />
...
const columns: any = ([
  ...
  {
    name: 'Endpoint', align: 'left', label: 'Home URL', required: true,
    field: (row: any) => {
