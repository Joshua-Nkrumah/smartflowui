<template>
    ...
    <div style="margin-top: -4rem;" class="q-pa-lg">
      <div class="row">
        <div class="col-12 ">
          ...
          <q-table style="margin-top: 2rem;" title="Membership" flat :filter="searchKey"
            :loading="paginationUtil.loading" v-model:pagination="paginationUtil.pagination" :rows="memberList"
          ...
          <q-stepper v-model="step" vertical color="primary" animated flat>
            ...
            <q-step :name="2" title="Other Information" caption="Optional" icon="create_new_folder" :done="step > 2">
              <q-form @submit="onProcessMember" @reset="onReset">
                <div class="row q-col-gutter-x-lg  ">
                  <div class="col-md-6">
                    <q-select v-model="memberPayload.maritalStatus" :options="maritalStatusOptions"
                      label="Marital Status" lazy-rules transition-show="scale" transition-hide="scale"
                      behavior="default" :rules="[(val) => (val && val.length > 0) || 'Select Marital Status']">
  ...
  <q-dialog v-model="confirmUserDialog" persistent>
    <q-card>
...
const confirmUserDialog = ref(false);
