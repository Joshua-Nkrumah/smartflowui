      ...
      <div class="row q-col-gutter-x-xl wrap">
        <div class="col-12 col-md-12 col-lg-12">
          <short-search-box label="Apply Changes" icon="published_with_changes" v-model="searchKey"
            @toggleDialog="onDetermineConfirmationPrompt_" :on-refresh="loadRecords" :viewType="false" />
...
const userList = computed(() => {
  ...
  return sisStore.listofStudents.records.filter((user) => {
    return (
      ...
      (user.studentNumber && user.studentNumber.toLowerCase().includes(search)) ||
      (user.status && user.status.toLowerCase().includes(search))
    );
  });
});
