<template>
      ...
      <template #title>
        <div class="row q-col-gutter-x-md">
          <div class="row items-center">
            <q-item-label class="text-h6 text-weight-bold"><q-icon name="summarize" size="1.5rem"
                class="q-mr-md" />Students Report
            </q-item-label>
          </div>
        </div>
      </template>
      <template #sub-title>
        <div class="text-h7 text_weight_light">Generate reports for all students in your school</div>
      </template>
    </MainBottomHeader>
    <div style="margin-top: -4rem;" class="q-pa-lg">
      <div class="row q-col-gutter-x-md q-col-gutter-y-md">
        ...
        <div class="col-xs-12 col-md-8 col-lg-9">
          <short-search-box label="Generate Report" icon="ios_share" v-model="searchKey" :view-type="false"
            @toggleDialog="onshowCreateStudentDialog" :on-refresh="loadRecords" />
  ...
  <div class="col-xs-12 col-md-12 col-lg-12">
    ...
    <!-- <SimpleBarChart :data="chartData" :labels="chartLabels" title="Sales Data" /> -->
  </div>
          ...
          <div class="row q-col-gutter-x-lg  ">
            <div class="col-sm-6" style="min-width: 300px;">
              <q-select v-model="studentPayload.gender" :options="studentGenderOptions" label="Gender *" lazy-rules
                ...
                :rules="[(val: string | any[]) => val && val.length > 0 || 'Select the student\'s gender']"></q-select>
            </div>
            <div class="col-sm-6" style="min-width: 300px;">
          ...
          <div class="row q-col-gutter-x-lg  ">
            <div class="col-sm-12">
              <q-select v-model="studentStatus" :options="studentStatusOptions" label="Student Status *" lazy-rules
                transition-show="scale" transition-hide="scale" behavior="default"
                :rules="[(val: null) => val != null || 'Please select the student status']"></q-select>
            </div>
          </div>
          <q-stepper-navigation>
...
import SimplePieChart from './components/charts/SimplePieChart.vue.vue';  // Assuming PieChart is in components folder
...
const pieChartData = ref([
  { value: studentSummary.value.totalMales, name: 'Male' },
  { value: studentSummary.value.totalFemales, name: 'Female' },
]);

// Data for the bar chart (status distribution)
const barChartData = ref([
  { value: studentSummary.value.activeStudents, name: 'Active' },
  { value: studentSummary.value.inactiveStudents, name: 'Inactive' },
]);
