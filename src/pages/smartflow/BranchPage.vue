    ...
    <MainBottomHeader>
      ...
      <template #sub-title>
        <div class="row justify-between">
          <div>
            <div class="q-ma-md">
              <EmbeddedSearchBox label="Branches" icon="add_circle" v-model="searchKey" :on-refresh="loadRecords" />
            </div>
          </div>
          <div>
            <q-item-label class="text-h6 q-mt-md text-white text-weight-bold row self-center">
              <div class="justify-end">
                <q-icon name="domain" class="q-pr-sm text-white" color="black" size="1.8rem" />
                Branches
              </div>
            </q-item-label>
            ...
            <!-- <template v-slot:body-cell-actions="props">
              <q-td key="actions" :props="props" auto-width>
                <q-btn-dropdown color="grey-8" dense>
                  <q-list>
                    <q-item clickable @click="onDetermineApprovalPrompt(props.row as any, 'approve')"
                      v-if="props?.row?.status == 'ACTIVE' && props?.row?.approvalStatus == 'PENDING'">
                      <q-item-section side>
                        <q-icon name="o_check_circle" />
                      </q-item-section>
                      <q-item-section>
                        <q-item-label>Approve Request</q-item-label>
                      </q-item-section>
                    </q-item>
                    <q-item clickable @click="onDetermineApprovalPrompt(props.row as any, 'reject')"
                      v-if="props?.row?.status == 'ACTIVE' && props?.row?.approvalStatus == 'PENDING'">
                      <q-item-section side>
                        <q-icon name="o_block" />
                      </q-item-section>
                      <q-item-section>
                        <q-item-label>Reject Request</q-item-label>
                      </q-item-section>
                    </q-item>
                  </q-list>
                </q-btn-dropdown>
      ...
      <q-card color="primary" animated flat class="card-shadow">
        <q-card-section>
          <div class="row q-col-gutter-x-lg  ">
            ...
            <div class="col-md-6">
              <q-select v-if="isUpdate == false" v-model="requestPayload.requestType" :options="uamStore.unit"
                label="Unit/Department *" lazy-rules transition-show="scale" transition-hide="scale" behavior="default"
            ...
            <div class="col-md-6">
              <q-input v-model="requestPayload.customerName" label="Exception Title *" lazy-rules
                :rules="[val => val && val.length > 0 || 'Enter the exception title']" />
            </div>
            <div class="col-md-12">
              <q-input v-model="requestPayload.customerName" label="Description *" lazy-rules
                :rules="[val => val && val.length > 0 || 'Enter the description']" autogrow />
            </div>
            <!-- <div class="col-md-6">
              <q-select v-if="isUpdate == false" v-model="requestPayload.requestType" :options="uamStore.requestType"
                label="Request Type *" lazy-rules transition-show="scale" transition-hide="scale" behavior="default"
                option-value="value" option-label="label" emit-value map-options
                :rules="[val => val != null || 'Please select the request type']"></q-select>
            </div>
            <div class="col-md-6">
              <q-select v-if="isUpdate == false" v-model="requestPayload.paymentType" :options="uamStore.paymentType"
                label="Payment Type *" lazy-rules transition-show="scale" transition-hide="scale" behavior="default"
                option-value="value" option-label="label" emit-value map-options
                :rules="[val => val != null || 'Please select the payment type']"></q-select>
            </div>

            <div class="col-md-6">
              <q-input v-model="requestPayload.sourceAccount" label="Source Account *" lazy-rules
                :rules="[val => val && val.length > 0 || 'Enter the source account']" />
            </div>
            <div class="col-md-6">
              <q-input v-model="requestPayload.destinationAccount" label="Destination Account *" lazy-rules
                :rules="[val => val && val.length > 0 || 'Enter the destination account']" />
            </div>
            <div class="col-md-6">
              <q-input v-model="requestPayload.amount" label="Amount  *" lazy-rules :rules="[
                (val) => (val && Number(val) && val > 0) || 'Enter the amount'
              ]" />
            </div> -->
            <!-- <div class="col-md-12">
              <q-input v-model="requestPayload.appOwner" label="Narration *" lazy-rules
                :rules="[val => val && val.length > 0 || 'Enter the narration']" />
            </div> -->
          </div>
        </q-card-section>
      </q-card>
...
const requestList = computed(() => {
  ...
  return uamStore.branches.filter((app) => {
    return (
      (app.branchName && app.branchName.includes(search)),
      (app.branchCode && app.branchCode.toLowerCase().includes(search))
    );
  });
});
...
const columns: any = ([
  // { name: 'actions', align: 'center', label: 'Action' },
  {
    ...
    align: 'left',
    field: (row: any) => {
      return row?.branchName ?? 0;
    },
  },
  {
    name: 'BranchCode', align: 'center', label: 'Branch Code', field: (row: any) => {
      return row?.branchCode ?? 0;
    },
  }
]);
