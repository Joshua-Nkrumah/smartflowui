<template>
  ...
  <q-layout view="hHh Lpr lff" container style="min-height: 100vh" class="shadow-2 rounded-borders">
    <q-header class="q-py-sm header-shadow primary-color">
      <q-toolbar>
        <q-btn flat dense round icon="o_menu" @click="toggleLeftDrawer" />
        ...
        <q-toolbar-title>FBL-BEM</q-toolbar-title>
        <div class="q-pr-lg gt-sm">
          {{ fullIdentity }}
        </div>
        <q-btn color="white" label="Logout" icon="logout" style="color: #ff851b !important; font-weight: bold;"
          class="text-weight-bol" push size="sm" v-close-popup @click="onLogout" />
    ...
    <!-- <q-drawer class="left-sidebar-shadow" side="left" overlay elevated v-model="leftDrawerOpen" :width="280"
      show-if-above style="background-color: #ffffff">
    </q-drawer> -->
    <q-drawer show-if-above v-model="leftDrawerOpen" :width="280" side="left" bordered>
...
const fullIdentity = <any>ref(null);
const firstName = <any>ref(null);

fullIdentity.value = LocalStorage.getItem('firstName') + ' ' + LocalStorage.getItem('lastName') + ' - ' + LocalStorage.getItem('approvalStatus');
